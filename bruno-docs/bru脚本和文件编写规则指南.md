...
-   **HTTP 方法块 (**`get`**,** `post`**,** `put`**,** `delete` **等)**： 定义请求的 URL 及其主要配置（如 body 类型和 auth 类型）。
...
-   **参数与请求头块 (**`query`**,** `headers`**)**： 直接以键值对形式编写。
...
-   **请求体块 (**`body:*`**)**： 支持多种数据格式，如 `body:json`、`body:text`、`body:xml`、`body:form-urlencoded` 等。如果声明为 JSON，内部可以直接写原生的 JSON 代码。
...
-   **鉴权块 (**`auth:*`**)**： 例如声明了 `auth:bearer` 后，需要提供对应的 token 配置。支持 `inherit`（继承父级文件夹配置）或使用变量如 `{{token}}`。
...
-   **引入外部库**： 你可以直接 `require()` 引入 Node.js 内置库，甚至引入当前文件夹下的其它外部 `.js` 脚本文件：
